...
**一套运行在本地的通用知识问答系统。装进 AI 编程工具，你的书就是你的知识库。**

你读过的书，不可能全记住。不同书里对同一件事的说法可能不一样，你也很难一一对照。这个 Skill 就是帮你做这件事的：把你的书、笔记、文献放进本地文件夹，它自动建立索引。之后你直接向 AI 提问，它会从所有书中找到相关内容，把不同来源的记载摆在一起对比，每一段话都标注来自哪本书，可以追溯到原文核实。

你不需要学任何新操作，在聊天窗口打字就行。装一个 Skill，换一套书，就是一个新的知识库。
